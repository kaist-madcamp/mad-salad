...
class TransactionController {
  static income = async (req: Request, res: Response) => {
    ...
    try {
      await prisma.$transaction([newAcct, trans]);
    } catch (error) {
  ...
  static receive = async (req: Request, res: Response) => {
    ...
    if (!tran) {
      res.json({
        ok: false,
        error: "no transaction Id  this should not happen",
      });
      return;
    }
    ...
    try {
      ...
      res.json({
        ok: false,
        error: "creating failed. try 1 more time, or deny it",
      });
      return;
    }
  ...
  static updateOneTransaction = async (req: Request, res: Response) => {
    ...
    if (!transactionId) {
      res.json({
        ok: false,
        error: "null paremeter or no transId",
      });
      return;
    }
    ...
    if (!transExist) {
      res.json({
        ok: false,
        error: "no such transId",
      });
      return;
    }

    if (
      !(amount || content || categoryName || type || createdAt || accountId)
    ) {
      res.json({
        ok: false,
